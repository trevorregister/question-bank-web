...
export default class AssignmentResponses {
  ...
  async getForStudent(
    responseId: string,
  ): Promise<GetResponseForStudentResponse> {
    const { data } = await this.client.get(`/responses/${responseId}`)
    return data
  }

  async submitResponsesToQuestions({
    responseId,
    responses,
  }: SubmitResponsesRequest): Promise<GetResponseForStudentResponse> {
    const { data } = await this.client.patch(
      `/responses/${responseId}/submit-responses`,
      { responses: responses },
    )
    return data
  }
