...
export default class Activities {
    ...
    async create({name, sections}: CreateActivityRequest): Promise<GetActivityResponse>{
        const { data } = await this.client.post('/activities', {name: name, sections: sections})
        return data
    }
    ...
    async updateActivity({activityId, name, sections, tags}: UpdateActivityRequest): Promise<GetActivityResponse>{
        const payload = {name: name, sections: sections, tags: tags}
        const { data } = await this.client.patch(`/activities/${activityId}`, payload)
        return data
    }
}